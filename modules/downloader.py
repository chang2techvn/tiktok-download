...
class Downloader(Tiktok):
    ...
    def read_links_file(self):
        #Handling the scenario where the file not found
    ...
    def __input_url(self, link):
        # Assuming `_wait_for_element_clickable` is defined elsewhere
    ...
    def __get_hd_link(self):
        ...
        # Call the __json_processing method to request using the extracted token value
    ...
    def __download_video(url, save_path):
        try:
            # Send a GET request to the specified URL with streaming enable
            ...
            with open(save_path, 'wb') as file:
                # Iterate through the response in chunks and write to the file
                for chunk in response.iter_content(chunk_size=8192):
                    file.write(chunk)

            print('Video has been successfully download and saved\n')
    ...
    def download(self, link, mode='hd'):
        ...
        # Calling the methods to input the link into the search field
        self.__input_url(link)

        #Check the method for video quality
        if mode == 'hd':
        ...
        # Create the full file save path by joining the save_path and the filename
