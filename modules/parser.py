...
class Parser(Tiktok):
    ...
    def __parsing_processing(self, selector, timeout=5):
        ...
        video_links = self.driver.find_element(By.XPATH, selector)
        ...
        while True:
            ...
            #On each interation, we update the value of 'prev_video_count'
    ...
    def login(self):
        ...
        if self.__cookies_file_exists(self.cookies_file_path):
            ...
            try:
                ...
                #Wait for the login button to because indicating successful login
                self._wait_for_element_located(By.CSS_SELECTOR, 'button[data-e2e="login-button"]')

                #Get the current session coockies and save them to a file
    ...
    def parse_by_keyword(self, key, mode='top'):
        ...
        if mode == 'top':
            # We'll parse links from the top tab
            video_links = self.__parsing_processing(top_tab_xpath,15)
        else:
            # We'll parse links from the videos tab
